...
def zero_one_knapsack_lp(weights, values, cap, verbose=False):
    ...
    for i, flag in enumerate(flags):
        exp.append(flag * weights[i])
        exp_1.append(flag * values[i])
    cons_exp = [sum(exp) <= cap]
    solver.add(cons_exp)
    solver.maximize(sum(exp_1))
    ...
    if result == sat:
        ...
        if verbose:
            print("\n".join([f"Index: {index}, Weight: {weights[index]}, Value: {values[index]}, Amount: {model[flag]}"
                             for index, flag in enumerate(flags) if int(str(model[flag])) >= 1]))

        return True, sum([values[index] * int(str(model[flag])) for index, flag in enumerate(flags) if int(str(model[flag])) >= 1])

    return False, result
    # raise ("exercise 16: please fill in the missing code.")
    # @end
...
if __name__ == '__main__':
    ...
    print("在大数集上dp的性能明显优于lp")
    # raise "exercise 17: please fill in the missing code.")
    # @end

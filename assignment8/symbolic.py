...
def symbolic_stmt(memory, stmt, rest_stmts, results):
    if isinstance(stmt, StmtAssign):
        ...
        p2 = mp.Process(target=symbolic_stmts, args=(memory, stmt.else_stmts, results, ExprBop(stmt.expr.left, stmt.expr.right, bop)))
        p1.start()
        p2.start()
        p1.join()
        p2.join()
...
def expr_2_z3(expr):
    ...
    # Your code here：
    # 比如，得到的x == 1
    # 约束，其数据结构表示为ExprBop(ExprVar("x"), ExprNum(1), Bop.EQ)， 你需要把它转换成
    # Int("x") == 1
    if isinstance(expr, ExprNum):
        return expr.num
    ...
    if isinstance(expr, ExprBop):
        ...
        # return ExprBop(expr_2_z3(expr.left), expr_2_z3(expr.right), expr.bop)

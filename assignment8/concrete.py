...
def interpret_stm(memory, stmt):
    ...
    if isinstance(stmt, StmtAssign):
        flag = 0
        for arg in memory.args:
            if arg == stmt.var:
                flag = 1
                break
        if flag != 1:
            memory.args.append(stmt.var)
        ...
        if interpret_expr(memory, stmt.expr):
            return interpret_stmts(memory, stmt.then_stmts)
        return interpret_stmts(memory, stmt.else_stmts)

    elif isinstance(stmt, StmtWhile):
        if interpret_expr(memory, stmt.expr):
            return interpret_stm(interpret_stmts(memory, stmt.stmts), stmt)
...
class TestConcrete(unittest.TestCase):
    def test_interpret_exp(self):
        ...
        exp2 = ExprBop(ExprBop(ExprNum(10), ExprNum(2), Bop.DIV),
                       ...
                       Bop.NE)
        # m != n
        exp3 = ExprBop(ExprVar("m"), ExprVar("n"), Bop.NE)
